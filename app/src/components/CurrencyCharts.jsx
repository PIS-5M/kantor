...
function Charts () {
    ...
    const [startDate, setStartDate] = useState(moment().subtract(range, 'days').format('YYYY-MM-DD'));
    ...
    const fetchExchangeRates = async (e, currency, topCount) => {
        try {
          const response = await fetch(`http://api.nbp.pl/api/exchangerates/rates/a/${currency}/last/${topCount}/`);
          if (!response.ok) {
...
setAllDatesInRange(tempAllDatesInRange)};
      ...
      useEffect((e) => {getStartDate(e)}, [range]);
      useEffect((e) => {fetchExchangeRates(e, currency, range)}, [currency, startDate]);
      useEffect(() => {getFullData()}, [dates, exchanges]);

      const data = {labels: allDatesInRange,
        datasets: [
            {
              label: 'Wartość',
              data: filledData,
      ...
      return(
        <div>
            ...
            <Input className='inputStyle2' type="select" onChange={(e) => {setCurrency(e.target.value)}}>
            ...
            <Input className='inputStyle2' type="select" onChange={(e) => {setRange(e.target.value)}}>
